...
<script>
  ...
  import {Popup, querystring} from 'vux';
  ...
  export default {
    ...
    methods: {
      ...
      getCarNOInfo: function (car) { // 编辑时获取车牌信息
        this.realName = car.driverName;
        this.phone = car.driverPhone;
        if (car && car.type === 0) { // 普通车牌
          let carNO = car.carNO;
          this.shortProvince = carNO.substring(0, 1); // 车牌省份简称
          this.commonCarNO = carNO.substring(1, carNO.length); // 车牌后六位
        } else { // 特殊车牌
          this.specialCarNO = car.carNO;
          // 切换到特殊车牌页面
          this.currentTab = 2;
        }
      },
      ...
      checkPhone: function (phone) {
        ...
        if (!regp.test(this.phone)) {
          Toast(phoneError.errorMsg); // TODO 提醒方式
          return false;
        } else {
          return true;
        }
      ...
      saveCarNOInfo: function () {
        ...
        let postData = {
          id: carId,
          type: type,
          driverName: this.realName,
          driverMobile: this.phone,
          carNum: carNum
        };
        let postDataStr = querystring.stringify(postData);
        this.$JHttp.put(window.homeURL + '/sharedPark/car/' + carId, postDataStr, {
          headers: {
