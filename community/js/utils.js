...
function newPost(uRlNoRoot, jsonData, fuc, fucParam,type) {
    NewDoAjax(uRlNoRoot, jsonData, undefined, true, fuc, fucParam,type);
}

function DoAjax(uRlNoRoot, jsonData, loadingContent, async, fuc, fucParam) {
    lockSubmit();
    showLoading(loadingContent);
    var defCommunityId = getRouterParam('defRoomId');
    var defRoomId = getRouterParam('defRoomId');
    if (defCommunityId == undefined || typeof (defCommunityId) == undefined || defCommunityId == null) {
        defCommunityId = "";
    }
    if (defRoomId == undefined || typeof (defRoomId) == undefined || defRoomId == null) {
        defRoomId = "";
    }

    uRlNoRoot = uRlNoRoot + ((uRlNoRoot.indexOf("?") == -1) ? "?" : "&") + "sign=9c19d8b9dfab1d41a99bc54dced3a9795dd033c8&fieds=version&version=2";
    if (defCommunityId != '') {
        uRlNoRoot = uRlNoRoot + '&defCommunityId=' + defCommunityId;
    }
    if (defRoomId != '') {
        uRlNoRoot = uRlNoRoot + '&defRoomId=' + defRoomId;
    }
    if (async == undefined) {
        async = false;
    }
    ...
    var req = $.ajax({
        // headers: {
        //     token: getRouterParam('token'),
        //     companyCode: getRouterParam('companyCode'),
        //     defCommunityId: getRouterParam('defCommunityId')
        // },
        type: "post",
        url: _HOME + uRlNoRoot,
        async: async,
...
function NewDoAjax(uRlNoRoot, jsonData, loadingContent, async, fuc,  fucParam, type) {
    lockSubmit();
    showLoading(loadingContent);

    //uRlNoRoot = uRlNoRoot + ((uRlNoRoot.indexOf("?")==-1)?"?":"&")+ (getRouterParam('defcommunityId')? "defCommunityId="+getRouterParam('defcommunityId'):'')+(getRouterParam('defcommunityId')? "defRoomId="+getRouterParam('defRoomId'):'');
    if (async == undefined) {
        async = false;
    }
    if (type == undefined) {
