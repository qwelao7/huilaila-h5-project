  ...
  <div class="building_dynamic">
    ...
    <div class="building_content">
      <div class="building_img" @click="toDetail(buildList1)">
        ...
        <j-img :osskey="buildList1.img" :custom-width="345" :custom-height="200"></j-img>
      ...
      <popup v-model="showCommentField" height="100%" style="background-color: #ffffff">
        <div class="commentPopup">
          <x-header
            ...
            <a slot="right" @click="subComment(buildList1)">发送</a>
          </x-header>
          <group>
      ...
      addPraise (value) {
        if (!this.token) {
          ...
          if (value.value === 'survey') {
            type = 6
          } else if (value.value === 'commiteeAchievement' || value.type === '18') {
            type = 7
          } else {
            type = 5
          }
...
//        let value = this.topic;
        ...
        let type;
        if (value.type === '12' || value.value === 'buildLive') {
          type = 7
        } else if (value.type === '13' || value.value === 'announcement') {
          type = 9
        } else if (value.type === 'survey' || value.value === 'survey') {
          type = 3
        } else if (value.type === '14' || value.value === 'project') {
          type = 8
        } else if (value.type === '18' || value.value === 'commiteeAchievement') {
          type = 12
        }
        let neighborId = '';
        if (value.type === 'survey') {
...
//          }
      ...
      toDetail (val) {
        ...
        if (val.type === 'survey') {
          ...
          this.$router.push('/buildLiveDetail/' + val.id)
        } else if (val.type === '13') {
          this.$router.push('/announcementDetail/' + val.id)
        } else if (val.type === '14') {
          this.$router.push('/projectDetail/' + val.id)
        } else if (val.type === '18') {
          this.$router.push('/commiteeAchievementDetail/' + val.id)
        } else {
