<template>
  <div class="commonHeader treatyDetail">
            ...
            <div class="commentList no-margin-top">
              <group class="commentCell" v-show="commonLists.topicCommentNumber > 0">
                <cell-box class="commentTotal">
                  <span>共{{commonLists.topicCommentNumber}}条建议</span>
                </cell-box>
    ...
    <div v-transfer-dom>
      <popup v-model="showCommentField" height="100%" style="background-color: #ffffff">
        <div class="commentPopup">
          ...
          <group>
            <x-textarea :max="250" v-model="commentText" class="commentField" placeholder="说点什么吧" :show-counter="false" :rows="1" autosize></x-textarea>
          </group>
        </div>
      </popup>
    </div>
    ...
    methods: {
      ...
      getBuildDetail () {
        ...
        that.$JHttp({
          ...
          url: window.baseURL + '/focusInfo/focusDetail?fid=' + id
        }).then(res => {
