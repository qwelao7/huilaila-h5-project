...
//        saveMoney: 0,
    ...
    beforeRouteEnter (to, from, next) {
      next(vm => {
        if (from.path.indexOf('/selectReceiveAddress') !== -1 && Object.keys(vm.$store.state.confirmOrdersData).length > 0) {
          // 回显缓存数据
          vm.$nextTick(() => {
            vm.orderData = vm.$store.state.confirmOrdersData.orderData
            vm.productsLists = vm.$store.state.confirmOrdersData.productsLists
            vm.showDelivery = vm.$store.state.confirmOrdersData.showDelivery
            vm.deliveryFree = vm.$store.state.confirmOrdersData.deliveryFree
            // vm.cantPay = vm.$store.state.confirmOrdersData.cantPay
            vm.TypeArr = vm.$store.state.confirmOrdersData.TypeArr
            vm.payType = vm.$store.state.confirmOrdersData.payType
            vm.productInfo = vm.$store.state.confirmOrdersData.productInfo
            vm.userAddressInfo = vm.$store.state.confirmOrdersData.userAddressInfo
            vm.noAddress = vm.$store.state.confirmOrdersData.noAddress
            vm.isConflict = vm.$store.state.confirmOrdersData.isConflict
            // vm.flag = vm.$store.state.confirmOrdersData.flag
            vm.abnormalStatus = vm.$store.state.confirmOrdersData.abnormalStatus
            vm.redPacketIdList = vm.$store.state.confirmOrdersData.redPacketIdList
            vm.redListStr = vm.$store.state.confirmOrdersData.redListStr
...
//        console.log(111, this_.redPacketIdList)
      ...
      chooseAddress () {
        ...
        let data = {
          ...
          redPacketIdList: this.redPacketIdList,
          redListStr: this.redListStr
        }
        this.$store.commit('updateConfirmOrdersData', data)
        let arr = [];
        let addrId = ''
