                  ...
                  <li @click="choose(item, redLists, index)" v-for="(item, index) in redLists">
                    ...
                    <div class="content">
                      ...
                      <div class="name">
                        ...
                        <span>有效期：{{item.fromDate}} 至 {{item.toDate}}</span>
                      </div>
                    </div>
                  </li>
                </ul>
...
<script>
  ...
  export default {
    ...
    created () {
      this.createDelivery()
      // 选择地址之后需要回显时间、红包和留言
      this.serverTimes = this.productItem.serverTimes
      this.redPacketObj = this.productItem.redPacketObj
      this.msgForStore = this.productItem.message
    },
    methods: {
      createDelivery () {
              ...
              res.days.forEach(d => {
                ...
                if (new Date(d).toDateString() === new Date().toDateString()) {
                  ...
                  res.timeNormal.forEach(t => {
                    ...
                    this_.timeTomorrow.push(obj2)
                  })
                }
              })
            })
            this_.timeData = this_.timeData.concat(arr)
            this_.timeData = this_.timeData.concat(this_.timeToday)
            this_.timeData = this_.timeData.concat(this_.timeTomorrow)
          }
        }).catch(e => {
          console.log(e)
        })
