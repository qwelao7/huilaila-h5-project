<template>
  <div class="order-detail commonHeader">
    <view-box body-padding-top="1.253333rem" body-padding-bottom="0">
      ...
      <span v-if="subHandle === '0'">
        <div class="form-preview order-desc">
          <div class="form-preview-item">
            <label class="form-preview-label">服务状态：</label>
          ...
          <div class="form-preview-item">
            <label class="form-preview-label">服务编号：</label>
        ...
        <div class="delivery-info">
          <div class="info">
            <div class="info-item">
              <div class="left" v-text="'联系人：' + name"></div>
        ...
        <div class="order-info">
          <div class="title border" v-text="customName"></div>

          <div class="form-preview info">
            <div class="form-preview-item">
              <label class="form-preview-label ">预约楼盘：</label>
              <span class="form-preview-value" v-text="communityName"></span>
            </div>
            <div class="form-preview-item" v-if="orderRemark">
              <label class="form-preview-label">车牌尾号：</label>
        ...
        <div class="feedback-info" v-if="aLifeOrderTaskInfoV">
          ...
          <div class="info">
            ...
            <div class="info-img" v-if="aLifeOrderTaskInfoV.images && aLifeOrderTaskInfoV.images.length > 0">
              <img class="previewer-demo-img" v-for="(img, index) in aLifeOrderTaskInfoV.images" :src="img" @click="showPreviewer(index)"/>
            </div>
          </div>
        </div>
        ...
        <div :style="{ 'margin-bottom': showBottomButtonCommunityOrder ? '1.867rem' : '0.533rem' }" ></div>

        <div class="button-cancel" v-if="orderStatus === 1" @click="isCancelOrder">取消服务</div>
      ...
      <span v-if="subHandle === '1'">
        ...
        <div class="form-preview order-desc">
          <span class="icon-detail" @click="goHandleDetail" v-if="showHandleDetail()">
        ...
        <div class="afterSales-info" v-if="hasAfterSalesLog" @click="goAfterServiceList">
          <span>售后记录</span>
        ...
        <div class="order-info">
          ...
          <product-list :productList="productList" :isAfterService="isAfterService" :orderId=orderId></product-list>
        ...
        <div class="comment-info" v-if="appraise">
          ...
          <div class="info">
            ...
            <div class="comment-time" v-text="appraise.appraiseTime"></div>
            <div class="comment-img" v-if="appraise.appraiseImgList && appraise.appraiseImgList.length > 0">
              <img class="previewer-demo-img-comment" v-for="(img, index) in appraise.appraiseImgList" :src="img" @click="showPreviewerComment(index)"/>
            </div>
            <div class="reply" v-if="appraise.isReply === '1'">
              <div class="reply-text" v-text="'回复：' + appraise.replyContent"></div>
              <div class="reply-time" v-text="appraise.replyTime"></div>
        ...
        <div class="button-cancel" v-if="orderStatus === 1 && moneyType === 0" @click="isCancelOrder">取消服务</div>

        <div class="button-double" v-if="orderStatus === 0 && subHandle === '1'">
          <div class="button-double-item button-double-cancel" @click="isCancelOrder">取消服务</div>
          <div class="button-double-item button-double-pay" @click="goPay">付款</div>
        </div>

        <div class="button-cancel" v-if="orderStatus === 1 && moneyType > 0 && subHandle === '1' && isAfterSales === 0 && isAftersalesIng === 0" @click="applyCanCelOrder">申请取消服务</div>
      </span>
    ...
    <div v-transfer-dom>
      <x-dialog v-model="showScrollBox" class="qr" hide-on-blur :dialog-style="{'max-width': '100%', width: '84%'}">
        <div class="order-detail-qrcode-swapper">
          <p class="order-detail-qrcode-header">服务二维码</p>
    ...
    <x-dialog class="cancel-order-wrapper" v-model="cancelOrderModalShow" :dialog-style="cancelOrderDialogStyle" hide-on-blur>
      <span class="delete-info vux-1px-b">你确定取消服务么?</span>
    ...
    <div v-transfer-dom>
      <previewer :list="imgListComment" ref="previewerComment" :options="optionsComment"></previewer>
    </div>
  </div>
</template>
<script>
  import { ViewBox, XHeader, TransferDom, XDialog, Qrcode, querystring, Previewer } from 'vux'
  ...
  export default {
    ...
    data () {
      return {
        ...
        showBottomButtonCommunityOrder: false,
        title: '服务详情',
        orderId: '',
        storePhone: '',
        showScrollBox: false,
        ...
        isPay: 0,
        hasLogistics: false,
        hasAfterSalesLog: 0, // 是否显示售后记录cell
        logisticsList: [],
        name: '',
        phone: '',
        address: '',
    ...
    methods: {
      getOrderDetail (loaded) {
        ...
        _this_.$JHttp({
          ...
          if (res.status === 100) {
            _this_.communityName = res.data.communityName
            _this_.customName = res.data.customName
            ...
            _this_.isAfterSales = res.data.isAfterSales
            _this_.isAftersalesIng = res.data.isAftersalesIng
            if (_this_.isAfterSales === 0 && (_this_.orderStatus === 3 || _this_.orderStatus === 4)) {
              _this_.isAfterService = true
            }
            ...
            _this_.hasAfterSalesLog = res.data.hasAfterSalesLog
            _this_.closeType = res.data.closeType
            ...
            if (_this_.aLifeOrderTaskInfoV && _this_.aLifeOrderTaskInfoV.images && _this_.aLifeOrderTaskInfoV.images.length > 0) {
              _this_.aLifeOrderTaskInfoV.images.forEach(function (v, i) {
                _this_.imgList.push({ src: v });
              })
            }
            _this_.appraise = res.data.appraise
            if (_this_.appraise && _this_.appraise.appraiseImgList && _this_.appraise.appraiseImgList.length > 0) {
              _this_.appraise.appraiseImgList.forEach(function (v, i) {
                _this_.imgListComment.push({ src: v })
              })
            }
            _this_.oldAppraise = res.data.oldAppraise
            if (_this_.subHandle === '1') {
              if (_this_.orderStatus === 3 || (_this_.orderStatus === 4 && (_this_.appraise && _this_.appraise.appraiseAgain === '0')) || (_this_.orderStatus === 1 && _this_.moneyType === 0) || (_this_.orderStatus === 0 && _this_.subHandle === '1') || (_this_.orderStatus === 1 && _this_.moneyType > 0 && _this_.subHandle === '1' && _this_.isAfterSales === 0 && _this_.isAftersalesIng === 0)) {
                _this_.showBottomButton = true
              } else {
                _this_.showBottomButton = false
              }
            } else {
              if (_this_.orderStatus === 1 || _this_.orderStatus === 3 || (_this_.orderStatus === 4 && (_this_.appraise && _this_.appraise.appraiseAgain === '0'))) {
                _this_.showBottomButtonCommunityOrder = true
              } else {
                _this_.showBottomButtonCommunityOrder = false
              }
            }
          }
        }).catch(e => {
      ...
      showHandleDetail () {
        if (this.orderStatus > 1) {
          if (this.orderStatus === 5 && (this.closeType !== 3 && this.closeType !== 5)) {
            return false
      ...
      showPreviewerComment (index) {
        this.$refs.previewerComment.show(index);
      },
      goHandleDetail () {
        this.$router.push('/handleDetail/' + this.orderId)
      },
      goAfterServiceList () {
        this.$router.push('/afterServiceList?orderId=' + this.orderId)
      },
      goCommentDetail () {
        this.$router.push({
          path: '/evaluateDetail',
      ...
      applyCanCelOrder () {
        this.$router.push({ path: '/cancelOrder/' + this.orderId, query: { type: 1 } })
      },
      cancelOrder () {
        let _this_ = this
        _this_.$JHttp({
