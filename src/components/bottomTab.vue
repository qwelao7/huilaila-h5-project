<template>
  <div class="bottomTab" v-model="index">
    <tabbar @on-index-change="onItemClick">
      <tabbar-item :selected="path === '/life/recommend'" link="/life/recommend">
      ...
      <tabbar-item :selected="path === '/service/new'" link="/service">
...
//        console.log(this.path);
      ...
      pub_newThings () {
        ...
        if (!token) {
          ...
          // 默认发布到新鲜事
          this.$router.push({path: '/pub_newThings', query: {type: 6}});
        }
      },
      pub_activity () {
        this.publish = false;
