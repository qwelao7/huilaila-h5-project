  ...
  export default {
    ...
    methods: {
          ...
          if (res.status === 100) {
            ...
            if (addressList && addressList.length) {
              addressList.forEach(function (addressInfo) {
                ...
                addressInfo_.areaCode = addressInfo.code;
                addressInfo_.latitude = addressInfo.latitude;
                addressInfo_.longitude = addressInfo.longitude;
      ...
      showChangeHouseModal (house) {
        ...
        this.currentCommunityId = house.community.id;
        this.currentCommunityName = house.community.name;
        this.currentAreaCode = house.areaCode;
        this.currentLatitude = house.latitude;
        this.currentLongitude = house.longitude;
      ...
      select () {
        ...
        this.$JHttp({
          ...
          if (res.status === 100) {
            localStorage.setItem('communityId', this.currentCommunityId);
            localStorage.setItem('communityName', this.currentCommunityName);
            // 更新经纬度及areaCode
            localStorage.setItem('longitude', this.currentLongitude);
            localStorage.setItem('latitude', this.currentLatitude);
            localStorage.setItem('areaCode', this.currentAreaCode);
            // 同步更新停留小区,并更新请求头
            initialAxios();
            // 重新获取列表数据
            _this_.getData();
      ...
      operate (addressItem) {
        ...
        if (status === '1') {
          this.$router.push('/houseDetail/' + addressItem.isDefault);
        } else {
