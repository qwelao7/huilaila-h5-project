<template>
  <div class="activityDetail commonHeader">
    ...
    <!--举报开始-->
    <div v-transfer-dom>
      <actionsheet :menus="reportMenu" @on-click-menu="onReport" v-model="showMenus" show-cancel></actionsheet>
    </div>
    <!--举报结束-->
    <div v-transfer-dom>
...
//              this.showApplyUserAvatar = false
            ...
            let shareData = {
              ...
              desc: _this.detailObj.content, // 分享描述
              link: link,
...
//              this.$router.push('/signUp/' + val.activityId)
              if (val.joinOtherCommunity === 0) {
                this.$router.push('/signUp/' + val.activityId)
              } else {
                if (this.currentUser.ownerAsset) {
                  let communityId = this.currentUser.ownerAsset.communityId
                  let localCommunityId = this.detailObj.communityId
                  console.log(communityId === localCommunityId)
                  if (communityId === localCommunityId) {
                    this.$router.push('/signUp/' + val.activityId)
                  } else {
                    this.$vux.toast.show({
                      type: 'text',
                      text: '您还不是该园区的业主，先去绑定房号吧'
                    })
                  }
                } else {
                  this.$vux.toast.show({
