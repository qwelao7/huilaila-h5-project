<template>
  <div class="signUpPeople commonHeader">
    <view-box ref="viewBox" body-padding-top="1.253333rem" :body-padding-bottom="paddingBtm"><!--1.333333rem-->
      <x-header
        slot="header"
      ...
      <div style="height: 100%">
        <j-pull>
          ...
          <div slot="jpull-list">
            <div class="signUp_1" v-if="applyUserListInformation.applyUserCount && !applyUserListInformation.myself">
              <ul>
                <li class="others" v-for="item in applyUserListInformation.applyList">
                  <div v-for="value in item.userList">
                    <img :src="value.joinUserPhoto" alt="" v-if="value.joinUserPhoto">
                    <img src="../../../assets/images/default_avatar.png" alt="" v-if="!value.joinUserPhoto">
                    <span>{{value.joinUserName}}</span>
                  </div>
                </li>
              </ul>
            </div>
            <div class="signUp" v-if="applyUserListInformation.applyUserCount && applyUserListInformation.myself">
              <ul>
                <li v-for="item in applyUserListInformation.applyList" v-show="needDetail === 0">
                  <div class="signUpPeople_informaton" v-for="value in item.userList">
                    <div class="applyUser">
                      <div class="left">
                        ...
                        <div class="sp_name">
                          <p class="name" v-text="value.joinUserName"></p>
                          <!--<p class="sex" v-text="item.joinUserSex === '1' ? '男' : '女'"></p>-->
                          <p class="num">共{{item.joinUserCount}}人</p>
                        </div>
                      </div>
                      <div class="phone" v-show="value.joinUserPhone">
                        <a style="width: 100%;height: 100%;display: block;" :href="'tel:' + value.joinUserPhone"></a>
                      </div>
                    </div>
                    <div class="note" v-show="item.note" v-text="item.note"></div>
                  </div>
                </li>
                <li v-for="item in applyUserListInformation.applyList" v-show="needDetail === 1">
                  <div class="signUpPeople_informaton" v-for="value in item.userList">
                    <div class="applyUser">
                      <div class="left">
                        <div class="signUpPeople_pic">
                        ...
                        <div class="sp_name">
                          <div class="nameBox">
                            <p class="name">{{value.joinUserName}}</p>
                            <div :class="{sex_male: value.joinUserSex === '1', 'sex_female': value.joinUserSex === '2'}"></div>
                          </div>
                          <p class="num" v-if="value.joinUserBirthday">{{value.joinUserBirthday}}</p>
                        </div>
                      </div>
                      <div class="phone" v-show="value.joinUserPhone">
                        <a style="width: 100%;height: 100%;display: block;" :href="'tel:' + value.joinUserPhone"></a>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      paddingBtm () {
        let num = '0'
        if (!this.applyUserListInformation.myself && this.applyUserListInformation.needUserDetail) {
          num = '0'
        } else if (this.applyUserListInformation.myself && !this.totalFee) {
          num = '0'
        } else {
          num = '1.333333rem'
        }
