<template>
  <div class="commonHeader file-list-wrapper">
    <view-box :body-padding-top="level === 2 ? '1.253333rem' : 0" body-padding-bottom="0">
      <x-header
        v-if="level === 2"
        slot="header"
        ...
        <j-pull :refreshFunc="refreshData" :loadMoreFunc="loadMore">
          <div slot="jpull-list">
            <ul v-show="fileList.length">
              <li class="file-item vux-1px-b" @click="goLevelTwo(file.id, file.categoryName)" v-for="(file, index) in fileList" :key="index">
...
export default {
  ...
  methods: {
    getFileList (loaded) {
      ...
      _this_.$JHttp({
        method: 'get',
        url: window.baseURL + '/assetProfile/assetDecorationProfileCategory?' + querystring.stringify(params)
      }).then(res => {
        if (res.status === 100) {
