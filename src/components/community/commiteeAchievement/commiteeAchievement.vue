<template>
  <div class="commiteeAchievement commonHeader">
    <view-box ref="viewBox" body-padding-top=".94rem" body-padding-bottom="0">
      ...
      <j-pull :refreshFunc="refreshData" :loadMoreFunc="loadMore">
        <div slot="jpull-list">
          <div v-for="item in commiteeAchievementList" v-if="commiteeAchievementList.length">
  ...
  export default {
    ...
    data () {
      return {
        ...
        type: 18,
        hasMore: true,
        commiteeAchievementList: []
      }
    },
    created () {
      this.getCommiteeAchievementList();
    },
    methods: {
      refreshData (loaded) {
        this.curPage = 1;
        this.commiteeAchievementList = [];
        this.getCommiteeAchievementList(loaded);
      },
      loadMore (loaded) {
        this.curPage++;
        this.getCommiteeAchievementList(loaded);
      },
      getCommiteeAchievementList (loaded) {
        let that = this;
        ...
        that.$JHttp({
          ...
          if (res.status === 100) {
            ...
            data.forEach(res => {
              ...
              that.commiteeAchievementList.push(obj)
            });
          }
        }).catch(err => {
