        ...
        <div class="commodityRateList">
          <div class="commodityRating" v-for="(item, index) in productList">
            ...
            <div class="inputArea">
              ...
              <ul>
                <li class="showPic" v-for="(value,i) in item.picList" v-show="item.picList.length">
                  <img :src="value" alt="" @click="showDelPopup(i, index)">
                </li>
                <li class="addPic" v-show="item.picList.length < 4">
                  <img src="../../../assets/images/addpic-210.png" alt="" @click="addPic(item, index)">
                </li>
              </ul>
            </div>
            <div v-transfer-dom>
...
//                    })
      ...
      addPic (val, idx) {
        ...
        this.$wechat.chooseImage({
          ...
          sourceType: ['album', 'camera'],
          success: function (res) {
            val.picList = val.picList.concat(res.localIds);
            // let uploadData = new FormData()
            // uploadData.append('type', 'nei')
            let blobs = [];
            val.picList.forEach(function (imgId, index) {
              _this_.$wechat.getLocalImgData({
                localId: imgId, // 图片的localID
                success: function (res) {
                  let localData = res.localData; // localData是图片的base64数据，可以用img标签显示
                  ...
                  // uploadData.append('files', file.blob, file.fileName);
                  // 合成完最后一个,开始上传
                  if (index === val.picList.length - 1) {
                    _this_.uploadBlob(blobs, 'appraise', undefined, undefined, function (resList) {
                      // _this_.picLists = [];
                      // _this_.picLists = resList
                      // _this_.productList.forEach(r => {
                      //   if (val.productId === r.productId) {
                      //     val.appraiseImgs = resList
                      //   }
                      // })
                      _this_.productList[idx].appraiseImgs = resList
                    });
                    // _this_.$JHttp.post(window.uploadURL + '/upload', uploadData, {
                    //   headers: {
                    //     'Content-Type': 'multipart/form-data'
      ...
      delPic () {
        this.productList[this.productIndex].picList.splice(this.currentIndex, 1);
