        ...
        <div class="commodityRateList">
          <div class="commodityRating" v-for="item in productList">
            ...
            <div class="inputArea">
              ...
              <ul>
                <li class="showPic" v-for="(value,index) in item.appraiseImgs" v-show="item.appraiseImgs.length">
            ...
            <div v-transfer-dom>
              <actionsheet :menus="delMenu" @on-click-menu-menu="delPic(item)" v-model="showDeleteMenu"
      ...
      addPic (value) {
        ...
        this.$wechat.chooseImage({
          ...
          success: function (res) {
            ...
            let uploadData = new FormData()
            uploadData.append('type', 'nei')
            value.appraiseImgs.forEach(function (imgId, index) {
              _this_.$wechat.getLocalImgData({
                localId: imgId, // 图片的localID
                success: function (res) {
                  let localData = res.localData; // localData是图片的base64数据，可以用img标签显示
                  ...
                  uploadData.append('files', file.blob, file.fileName);
                  // 合成完最后一个,开始上传
                  if (index === value.appraiseImgs.length - 1) {
                    _this_.$JHttp.post(window.uploadURL + '/upload', uploadData, {
                      headers: {
                        'Content-Type': 'multipart/form-data'
                      }
...
//                        })
      ...
      delPic (val) {
        this.productList.forEach(res => {
          if (val.productId === res.productId) {
            val.appraiseImgs.splice(this.currentIndex, 1);
            val.picLists.splice(this.currentIndex, 1);
          }
        })
        this.$vux.toast.show({
          type: 'text',
