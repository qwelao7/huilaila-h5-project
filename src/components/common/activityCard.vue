  ...
  <div class="act-card">
    <div class="main-banner">
      ...
      <div class="introduce" @click="toDetail(information)">
        <ul>
          <li><p>{{information.activityName}}</p></li>
          <li>
            <span>活动开始时间：</span>
...
<script>
  ...
  export default {
    ...
    components: {
      XInput, Actionsheet, Group, XDialog, XHeader, Popup, XTextarea
    },
    props: {
      dataList: {
        type: Object,
        ...
        if (!this.token) {
          ...
          if (userId !== value.userInfo.id) {
            if (value.activityStatus === 2) {
              if (!value.joinOtherCommunity) {
                this.$router.push('/signUp/' + value.activityId)
              } else {
                if (this.currentUser.ownerAsset) {
                  let communityId = this.currentUser.ownerAsset.communityId
                  let localCommunityId = value.communityId
