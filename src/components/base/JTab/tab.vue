<template>
  <div class="vux-tab-warp">
    <div class="vux-tab-container" ref="tabContainer">
...
export default {
  ...
  computed: {
    barLeft () {
      if (this.hasReady) {
        let innerWidth = this.$refs.tabContainer.clientWidth;
        const count = this.scrollable ? (innerWidth / this.$children[this.currentIndex || 0].$el.getBoundingClientRect().width) : this.number
