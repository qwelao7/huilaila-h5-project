  ...
  <div class="cube-index-list">
    <cube-scroll
      ...
      <div class="cube-index-list-content" ref="content">
        ...
        <ul>
          <slot>
            <cube-index-list-group v-for="(group, index) in data" :key="index" :group="group" :value="value" @select="selectItem">
...
<script>
  ...
  export default {
    ...
    // },
    beforeUpdate () {
      if (this.listHeight[this.listHeight.length - 1] === 0) {
        this.title && this._caculateTitleHeight()
        this._calculateHeight()
        this.refresh()
      }
